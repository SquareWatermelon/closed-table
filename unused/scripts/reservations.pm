...
sub removeReservation{
    my ($day, $month, $year, $time, $table, $name) = @_;
    ...
    while(<DATE>){
        ...
        if($dateMatch eq $date and $timeMatch eq $time, and $tableMatch eq $table and $nameMatch eq $name){next;}
        print TEMPFILE $_;
    }
    close (DATE);
    close (TEMPFILE);
...
sub printPDF{
	...
	$pdf->saveas('/home/rem2174/dbmtester/date.pdf');#change this to suited location**	
}
